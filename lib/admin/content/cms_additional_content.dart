...
class _SaintsManagerScreenState extends State<SaintsManagerScreen> {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snap) {
        ...
        return AdminDataTable<SaintModel>(
          title: 'Saints',
          items: _saints,
          columns: ['Name (EN)', 'Name (AR)', 'Feast Date', 'Status'],
          ...
          onSearch: (q) => setState(() => _searchQuery = q),
          onAdd: () => setState(() => _editingItem = SaintModel(
            id: '',
            nameAr: '',
            nameEn: '',
            isPublished: true,
          ...
          onEdit: (item) => setState(() => _editingItem = item),
          onDelete: (id) => _deleteSaint(id),
          isLoading: snap.connectionState == ConnectionState.waiting,
          emptyMessage: 'No saints added yet',
        );
      },
    );
  }
  ...
  Widget _buildForm() {
    final item = _editingItem!;
    return ContentForm(
      title: item.id.isEmpty ? 'Add Saint' : 'Edit Saint',
      fields: [
        FormFieldConfig(
          key: 'nameEn',
        ...
        FormFieldConfig(
          key: 'nameCoptic',
          labelEn: 'Coptic Name (Optional)',
          labelAr: 'الاسم بالقبطية',
          value: item.nameCoptic,
        ),
        FormFieldConfig(
          key: 'feastDate',
          labelEn: 'Feast Date (MM-DD)',
          labelAr: 'تاريخ العيد',
          hintEn: 'e.g., 01-07',
          value: item.feastDate,
        ),
        FormFieldConfig(
          key: 'patronOfEn',
          labelEn: 'Patron Of',
          labelAr: 'الراعي ل',
          value: item.patronOfEn,
        ),
        FormFieldConfig(
          key: 'biographyEn',
          labelEn: 'English Biography',
          labelAr: 'السيرة الذاتية',
          multiline: true,
          minLines: 5,
          maxLines: 10,
          value: item.biographyEn,
        ),
      ],
      onSubmit: (data) => _saveSaint(data),
      onCancel: () => setState(() => _editingItem = null),
      submitLabel: item.id.isEmpty ? 'Create' : 'Update',
      isLoading: _isLoading,
    );
  ...
  Future<void> _saveSaint(Map<String, dynamic> data) async {
    setState(() => _isLoading = true);
    try {
      final item = _editingItem!;
      final userId = context.read<AuthCubit>().state.user?.uid ?? '';
      ...
      if (item.id.isEmpty) {
        await _fb.collection(AppConstants.saintsCollection).add(saint.toFirestore());
      } else {
        await _fb
            .collection(AppConstants.saintsCollection)
            .doc(item.id)
            .update(saint.toFirestore());
      }

      setState(() => _editingItem = null);
      _showSnackBar('Saint saved successfully');
    } catch (e) {
      _showSnackBar('Error: $e', isError: true);
    } finally {
      setState(() => _isLoading = false);
  ...
  Future<void> _deleteSaint(String id) async {
    try {
      await _fb.collection(AppConstants.saintsCollection).doc(id).delete();
      _showSnackBar('Saint deleted');
    } catch (e) {
      _showSnackBar('Error: $e', isError: true);
    }
  }
